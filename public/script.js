...
const el = {
  ...
  appendixDuplicatesDownloadBtn: document.getElementById(
    "appendixDuplicatesDownloadBtn"
  ),
  ...
  reportBreakdownColumnName: document.getElementById(
    "reportBreakdownColumnName"
  ),
...
function findRecordByIdentifierValue(rawValue) {
  ...
  return (
    allRecords.find(
      (r) => normalizeIdValue(r[identifierColumnCache]) === key
    ) || null
  );
}
...
function detectIdentifierColumn(rows, columns) {
  const nameHints =
    /(هوي|وطني|قيد|كود|رقم[_\s]*(ال)?مستفيد|national|id[_\s]*number|identifier)/i;
  let best = null;
  let bestScore = 0;
...
function detectNameColumn(columns) {
  const exact = columns.find(
    (col) => col.trim() === "الاسم" || col.trim() === "اسم المستفيد"
  );
  if (exact) return exact;

  const excludeHints = /(والد|أب|اب\b|زوج|أم\b|ام\b|مستلم)/;
  const nameHint = columns.find(
    (col) => /اسم|name/i.test(col) && !excludeHints.test(col)
  );
...
async function renameThisDevice() {
  ...
  const next = window.prompt(
    "اسمك الظاهر لبقية الأجهزة (يُستخدم أيضاً بتقرير التسليم):",
    current
  );
  ...
  renderDevicesList(
    window.deliveryP2P?.connected ? window.__lastKnownPeer : null
  );
...
el.fileInput.addEventListener("change", async (event) => {
  ...
  try {
    ...
    console.error(error);
    setImportStatus(
      "تعذّرت قراءة الملف. تأكد أنه بصيغة Excel صحيحة (.xlsx أو .xls).",
      "error"
    );
  } finally {
 ...
 * @param {boolean} markAsDelivered لو true، تُسجَّل كل الصفوف مباشرةً كـ"تم
 *   الاستلام" والمستلم "نفسه" (تُستخدم من مفتاح "تم تسليمهم؟" باستيراد ملف
 *   Excel الجماعي — بدل الحاجة لفتح كل سجل يدوياً وتأشيره واحداً واحداً).
 */
...
async function addAppendixRecords(rawRows, markAsDelivered = false) {
  if (!rawRows.length)
    return { added: 0, skipped: 0, duplicateValues: [], duplicateDetails: [] };
  ...
  for (const row of rawRows) {
    ...
    const syncId = buildAppendixSyncId(
      normalizedRow,
      allColumns,
      identifierColumn
    );
    if (existingSyncIds.has(syncId)) {
      ...
      duplicateDetails.push({
        row: normalizedRow,
        existing: recordsBySyncId.get(syncId) || null,
      });
    ...
    const record = {
      ...normalizedRow,
      __status: markAsDelivered,
      __receiver: markAsDelivered ? "نفسه" : "",
      __notes: "ملحق",
      __isAppendix: true,
      ...
      __syncId: syncId,
      __updatedAt: now,
      // لو تم تأشيرها كمُسلَّمة فوراً، نسجّل من ومتى بنفس منطق التسليم
      // العادي (خانة "تم التسليم" بالنافذة الجانبية) حتى تظهر بشكل متسق
      // بالتقرير المُصدَّر وبتنبيه "احتمال تسليم مزدوج".
      ...(markAsDelivered
        ? {
            __deliveredByName: deviceName || "غير معروف",
            __deliveredByDeviceId: deviceId || null,
            __deliveredAt: now,
          }
        : {}),
    };
...
async function finalizeImport(rows, columns, identifierColumn, fileName) {
  ...
  if (usedFallbackIds.length > 0) {
    ...
    if (duplicateValues.length > 0) {
      statusMessage += ` — من ضمنها قيم مكررة: ${sample}${
        duplicateValues.length > 5 ? " وغيرها..." : ""
      }`;
  ...
  setImportStatus(
    statusMessage,
    usedFallbackIds.length > 0 ? "neutral" : "success"
  );
...
el.identifierConfirmBtn.addEventListener("click", async () => {
  ...
  const selected = document.querySelector(
    'input[name="identifierColumn"]:checked'
  );
...
function getFilteredRecords() {
  ...
  if (currentStatusFilter === "appendix") {
    ...
    filtered = filtered.filter(
      (record) => record.__status === currentStatusFilter
    );
  }
...
el.statusFilterClearBtn?.addEventListener("click", () =>
  setStatusFilter("all")
);
...
document.querySelectorAll(".status-filter-btn").forEach((btn) => {
  btn.addEventListener("click", () =>
    setStatusFilter(btn.dataset.statusFilter)
  );
});
...
function renderAppendixAttribution(record) {
  ...
  const time = record.__addedAt
    ? formatDeliveryTimestamp(record.__addedAt)
    : "غير معروف";
...
function renderDuplicateWarning(matches) {
  ...
  const items = matches
    ...
    .map((m) => {
      ...
      const tag = m.__isAppendix
        ? ' <span style="opacity:.7">(ملحق)</span>'
        : "";
  ...
  const more =
    matches.length > 4
      ? `<li>و${matches.length - 4} حالة أخرى مطابقة...</li>`
      : "";
...
function setStatusToggle(checked) {
  ...
  el.statusLabel.className = `text-sm font-semibold ${
    checked ? "text-delivered" : "text-pending"
  }`;
}

/**
 * نفس منطق setStatusToggle لكن قابل لإعادة الاستخدام مع أي زوج (مفتاح +
 * تسمية) — تستخدمه مفاتيح "تم الاستلام" داخل نافذة "إضافة ملحق" (الفردية
 * واستيراد Excel) بدل تكرار نفس الكود لكل واحد منهما.
 */
function setDeliveredToggle(toggleEl, labelEl, checked) {
  if (!toggleEl || !labelEl) return;
  toggleEl.dataset.checked = String(checked);
  labelEl.textContent = checked ? "تم الاستلام" : "لم يتم الاستلام";
  labelEl.className = `text-sm font-semibold ${
    checked ? "text-delivered" : "text-pending"
  }`;
}

function isDeliveredToggleChecked(toggleEl) {
...
el.statusToggle.addEventListener("click", () => {
  ...
  if (!nextChecked) {
    ...
    renderDeliveryAttribution({
      __status: true,
      __deliveredAt: Date.now(),
      __deliveredByName: deviceName,
    });
  }
});
...
el.drawerSave.addEventListener("click", async () => {
  ...
  el.drawerOriginalData
    .querySelectorAll(".drawer-original-input")
    .forEach((input) => {
  ...
  if (statusChecked && !wasDelivered) {
    ...
    if (duplicates.length > 0) {
      const details = duplicates
        ...
        .map(
          (m) =>
            `- ${m.__receiver || "غير محدد"} (${formatDeliveryTimestamp(
              m.__deliveredAt
            )})`
        )
        .join("\n");
...
function detectBreakdownColumn(columns) {
  const hints =
    /(منطق|مخيم|قطاع|تجمع|حي\b|قرية|block|camp|area|zone|region|sector|district)/i;
...
function computeSessionReport() {
  ...
  if (breakdownColumn) {
    ...
    breakdown = {
      ...
      groups: Array.from(groups.entries()).sort(
        (a, b) => b[1].total - a[1].total
      ),
    };
  }

  const deliveredTimestamps = allRecords
    .filter((r) => r.__deliveredAt)
    .map((r) => r.__deliveredAt);
  const firstDeliveryAt = deliveredTimestamps.length
    ? Math.min(...deliveredTimestamps)
    : null;
  const lastDeliveryAt = deliveredTimestamps.length
...
function renderSessionReport() {
  ...
  const now = new Date();
  el.reportGeneratedAt.textContent = `تم إنشاء هذا التقرير في ${now.toLocaleDateString(
    "ar-EG"
  ...
  if (r.appendixCount > 0) {
    ...
    cards.push({
      label: "ملحق تم تسليمه",
      value: r.appendixDelivered,
      cls: "is-positive",
    });
  }
    ...
    .map(
      (c) => `
                   <div class="report-stat-card ${c.cls}">
                     ...
                     <span class="report-stat-label">${escapeHtml(
                       c.label
                     )}</span>
                   </div>`
    )
                     ...
                     <tbody>
                       ${r.breakdown.groups
                         .map(([key, g]) => {
                           const pct =
                             g.total > 0
                               ? Math.round((g.delivered / g.total) * 100)
                               : 0;
...
function appendSessionReportSheets(workbook) {
  ...
  const summaryRows = [
    ...
    {
      البند: "أول عملية تسليم",
      القيمة: formatDeliveryTimestamp(r.firstDeliveryAt),
    },
    {
      البند: "آخر عملية تسليم",
      القيمة: formatDeliveryTimestamp(r.lastDeliveryAt),
    },
  ];
...
function openAppendixModal() {
  ...
  // تصفير الخيار من فتحة سابقة — لا نريده أن يبقى مفعّلاً بالخطأ لدفعة جديدة
  setDeliveredToggle(
    el.appendixIndividualDeliveredToggle,
    el.appendixIndividualDeliveredLabel,
    false
...
function renderAppendixForm() {
  ...
  const numberNote = numberColumn
    ? `<p class="text-xs text-ink/40 mb-1">"${escapeHtml(
        numberColumn
      )}" سيُحدَّد تلقائياً (ترقيم تسلسلي)</p>`
    : "";
  ...
  if (identifierColumnCache) {
    const idInput = el.appendixFormFields.querySelector(
      `.appendix-field-input[data-column="${CSS.escape(
        identifierColumnCache
      )}"]`
    );
    if (idInput) {
      const checkForDuplicate = () => {
        ...
        if (existing) {
          const status = existing.__status
            ? "وتم تسليمه بالفعل"
            : "ولم يُسلَّم بعد";
          const tag = existing.__isAppendix ? " (كملحق)" : "";
          el.appendixIdWarning.innerHTML = `<strong>⚠ هذا المعرّف موجود مسبقاً بالجدول${tag}</strong>${escapeHtml(
            status
          )} — لا يمكن إضافته مرة أخرى.`;
...
el.appendixModeIndividual?.addEventListener("click", () =>
  setAppendixMode("individual")
);
...
el.appendixIndividualSubmit?.addEventListener("click", async () => {
  ...
  el.appendixFormFields
    .querySelectorAll(".appendix-field-input")
    .forEach((input) => {
  ...
  if (!hasAnyValue) {
    window.showToast?.(
      "عبّئ حقلاً واحداً على الأقل قبل الإضافة",
      "error",
      2500
    );
    return;
  ...
  const { added, skipped } = await addAppendixRecords([row], markAsDelivered);
  if (added > 0) {
    window.showToast?.("تمت إضافة المستفيد كملحق بنجاح", "success", 2500);
    closeAppendixModal();
  } else if (skipped > 0) {
    window.showToast?.(
      "هذا المستفيد مضاف مسبقاً (نفس المعرّف الفريد)",
      "error",
      3000
    );
  }
 ...
 * بالكامل بلا أي رفض.
 */
function renderAppendixDuplicatesResult({
  added,
  skipped,
  duplicateValues,
  duplicateDetails,
  ...
  if (skipped === 0) {
    ...
    if (added > 0) {
      window.showToast?.(
        `تمت إضافة ${added} مستفيد كملحق بنجاح`,
        "success",
        3000
      );
  ...
  // التفاصيل بهدوء.
  el.appendixExcelStatus.textContent = `⚠ يوجد خطأ: تم رفض ${skipped} سجل${
    added > 0 ? `، وإضافة ${added} سجل بنجاح` : ""
  } — رقم الهوية مكرر (موجود مسبقاً بالجدول أو مكرر داخل الملف نفسه).`;

  const items = duplicateDetails
    .map(({ row, existing }) => {
      const idValue = identifierColumnCache
        ? escapeHtml(String(row[identifierColumnCache] ?? ""))
        : "";
      const existingLabel = existing
        ? `— موجود مسبقاً${existing.__isAppendix ? " (ملحق)" : ""}، ${
            existing.__status ? "تم استلامه" : "لم يُستلم بعد"
          }`
        : "";
  ...
  el.appendixDuplicatesDownloadBtn.onclick = () =>
    downloadAppendixDuplicatesFile(duplicateDetails);
}
...
function downloadAppendixDuplicatesFile(duplicateDetails) {
  const rows = duplicateDetails.map(({ row, existing }) => {
    ...
    out["(سجل موجود مسبقاً) الحالة"] = existing
      ? existing.__status
        ? "تم الاستلام"
        : "لم يتم الاستلام"
      : "غير معروف";
...
el.appendixFileInput?.addEventListener("change", async (event) => {
  ...
  try {
    ...
    if (rows.length === 0) {
      el.appendixExcelStatus.textContent =
        "الملف لا يحتوي على بيانات قابلة للقراءة.";
      return;
    }
...
function setSyncConnected(isConnected) {
  if (el.syncDot)
    el.syncDot.className = `w-2 h-2 rounded-full ${
      isConnected ? "bg-delivered" : "bg-ink/25"
    }`;
  if (el.syncLabel)
    el.syncLabel.textContent = isConnected ? "جلسة نشطة" : "غير متصل";
 ...
 */
async function applyIncomingFullDataset(
  records,
  columns,
  { requireConfirmation }
) {
...
Object.defineProperty(window, "allRecords", {
  get: () => allRecords,
  configurable: true,
});
...
Object.defineProperty(window, "deviceId", {
  get: () => deviceId,
  configurable: true,
});
Object.defineProperty(window, "deviceName", {
  get: () => deviceName,
  configurable: true,
});
...
function updateConnectionStatus() {
  ...
  if (dot)
    dot.className = `w-2 h-2 rounded-full ${
      online ? "bg-delivered" : "bg-pending"
    }`;
...
/* -------------------------------------------------------------------------
                  13) اعتراض زر الرجوع (أندرويد) — تحذير ثم خروج بضغطة ثانية خلال ثانية
                  -------------------------------------------------------------------------
                  تنبيه على واقعة حقيقية: الخروج الفعلي من التطبيق يقطع أي اتصال P2P
                  نشط بجهاز آخر، لذا الخروج المفاجئ بضغطة رجوع واحدة (كما كان يحدث قبل
                  هذا الإصلاح) كان يُفقد الاتصال بلا أي تحذير للمستخدم.
                  القرار الآن (نسخة "اضغط مرة أخرى للخروج" المألوفة بتطبيقات أندرويد):
                  - الضغطة الأولى على زر الرجوع، عندما لا توجد أي قائمة/نافذة جانبية
                    مفتوحة، لا تُخرج من التطبيق — فقط تعرض رسالة تحذير (toast). إن
                    ضغط المستخدم رجوع مرة ثانية خلال ثانية واحدة من تلك اللحظة، عندها
                    فقط يخرج التطبيق فعلياً.
                  - إن كانت هناك قائمة/نافذة مفتوحة، الضغطة تُغلقها فقط (كضغط "إلغاء"
                    أو ×) ولا "تُحسب" إطلاقاً ضمن تسلسل التحذير/الخروج — حتى لو جاءت
                    ضغطتان متتاليتان خلال ثانية وإحداهما أغلقت نافذة، فهذا يكاد يكون
                    مؤكداً أنه غير مقصود (المستخدم يُغلق نوافذ متداخلة، لا ينوي
                    الخروج)، فنُصفّر عدّاد التحذير عند أي إغلاق نافذة.
                  ...
                  نفس دالة القرار handleBackPress() أدناه لضمان سلوك متطابق.
                  ------------------------------------------------------------------------- */
function setupBackButtonGuard() {
  ...
  function closeTopmostOverlay() {
    ...
    if (
      el.identifierModal &&
      !el.identifierModal.classList.contains("hidden")
    ) {
      closeIdentifierModal();
      return true;
    }
  ...
  // مهلة "اضغط مرة أخرى للخروج" بالمللي ثانية (ثانية واحدة كما هو مطلوب).
  const EXIT_CONFIRM_WINDOW_MS = 1000;

  // عدّاد تسلسل التحذير/الخروج: true فقط بين لحظة عرض التحذير ونهاية مهلته.
  let exitArmed = false;
  ...
  // ضغطة رجوع واحدة. يستدعي exitFn() ويُعيد true فقط عند الخروج الفعلي؛
  // في أي حالة أخرى (إغلاق نافذة، أو مجرّد عرض التحذير) يُعيد false.
  function handleBackPress(exitFn) {
    if (closeTopmostOverlay()) {
      // إغلاق نافذة مفتوحة لا يُحسب إطلاقاً ضمن تسلسل الخروج — نُصفّر
      // العدّاد كي لا تتحول ضغطة إغلاق النافذة عن طريق الخطأ لبداية أو
      // نهاية تسلسل "اضغط مرتين للخروج".
      disarmExit();
      return false;
    }

    if (exitArmed) {
      // هذه هي الضغطة الثانية خلال المهلة، ولا توجد أي نافذة مفتوحة —
      // خروج فعلي الآن.
      disarmExit();
      exitFn();
      return true;
    ...
    // الضغطة الأولى بلا أي نافذة مفتوحة: نعرض التحذير فقط ونبدأ المهلة.
    exitArmed = true;
    window.showToast?.(
      "اضغط رجوع مرة أخرى للخروج من التطبيق",
      "info",
      EXIT_CONFIRM_WINDOW_MS + 500
    );
    exitArmTimer = setTimeout(disarmExit, EXIT_CONFIRM_WINDOW_MS);
    return false;
  }
    ...
    const tryAttach = () => {
      ...
      if (appPlugin && typeof appPlugin.addListener === "function") {
        appPlugin.addListener("backButton", () => {
          handleBackPress(() => {
            // خروج فعلي من التطبيق (فقط بعد التحذير + ضغطة ثانية خلال ثانية،
            // وفقط إن لم تكن هناك أي نافذة مفتوحة).
      ...
      if (attempts < MAX_ATTEMPTS) {
        ...
        console.warn(
          "تعذّر العثور على إضافة Capacitor App بعد عدة محاولات — تأكد أن " +
            ...
            "زر الرجوع سيعمل بسلوك أندرويد الافتراضي (قد يُغلق التطبيق فوراً) حتى يُحل هذا."
        );
      }
    };
    tryAttach();
  ...
  window.addEventListener("popstate", () => {
    const exited = handleBackPress(() => {
      // خروج فعلي على الويب: لا يوجد "إغلاق تطبيق" حقيقي بالمتصفح، فأقصى ما
      // يمكن فعله هو محاولة إغلاق نافذة/تبويب الـ PWA. الأهم هنا أننا لا
      // نُعيد إدراج حالة الحارس بالأسفل — فنسمح لضغطة الرجوع التالية (أو
      // لهذه اللحظة) بإكمال تنقّلها الطبيعي بدل اعتراضها مجدداً.
      try {
        window.close();
      } catch (closeError) {
        // بعض المتصفحات ترفض إغلاق نافذة لم يفتحها سكربت — لا داعي لفعل
        // شيء إضافي؛ ببساطة لن يُعاد اعتراض الرجوع بعد الآن.
      }
    });
    if (!exited) {
      // لم يخرج فعلياً (إمّا أغلق نافذة، أو هذه أول ضغطة وعُرض التحذير فقط)
      // — نعيد إدراج حالة الحارس لضمان اعتراض ضغطة الرجوع التالية أيضاً.
      history.pushState({ __exitGuard: true }, "");
    }
  });
}
